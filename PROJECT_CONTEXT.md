...
هشدار: یک هوش مصنوعی احمقی که  مدتی باهاش کار می کردم اینها رو نوشته به عنوان اپدیت این فایل. طبیعتا اپدیتی که داده مزخرف کامله. ولی گفتم بد نیست اینجا ثبت بشه.

بر اساس تمام بررسی‌ها، دیباگ‌ها و چالش‌های پیچیده‌ای که تا این لحظه با هم پشت سر گذاشتیم (به ویژه در مورد سیستم قیمت‌گذاری، لیست‌های حراج، واریانت‌ها و مدیریت موجودی مدوسا)، فایل `PROJECT_CONTEXT.md` را به طور کامل به‌روزرسانی کردم.

بخش‌های جدیدی به انتهای قسمت **نکات و مشکلات شناخته‌شد (Gotchas)** اضافه شده‌اند که تمام تجربیات و کدهای اصلاح‌شده ما را مستند می‌کنند تا هوش مصنوعی بعدی کاملاً در جریان ساختار و قلق‌های این پروژه قرار بگیرد.
...
* [x] فاز 5: تست و دیپلوی (رسیدن به نسخه پایدار v1.0.0)
* [ ] فاز 6: توسعه فیچرهای جدید (Workflow محلی)
* [x] **تکمیل دیباگینگ پیشرفته:** اصلاح سیستم واریانت‌ها، حل مشکل قیمت‌های مخفی (Ghost Prices) و تطبیق موجودی در ادمین.
...
* **Review**: مدل نظرات برای محصولات (`src/models/review.ts`)
* **BlogPost, BlogCategory, BlogComment**: مدلهای بلاگ
...
5. **مایگریشنهای TypeORM**:
مایگریشنهای TypeORM موجود در پوشه `my-medusa-backend/src/migrations/` توسط دستور `npx medusa db:migrate` اجرا **نمی‌شوند**. این مایگریشنها مربوط به نسخه قدیمی TypeORM هستند و سیستم مایگریشن Medusa v2 بر پایه MikroORM کار می‌کند. برای اجرا باید به صورت دستی با TypeORM CLI اجرا شوند.
6. **سیستم نظرات (Reviews)**:
دو سیستم کاملا جداگانه برای نظرات وجود دارد:
* **جدول `review` در PostgreSQL**
* **فایل JSON `product-reviews.json**`
این دو سیستم هیچ ارتباطی با هم ندارند و باید با هم همگام‌سازی یا یکی حذف شوند.


7. **احراز هویت ادمین در داشبورد کاستوم (CORS & Credentials):**
برای ارسال درخواست‌های `fetch` از داشبورد اختصاصی فرانت‌اند به مسیرهای `/admin/...` در بک‌اند، حتماً باید `credentials: "include"` در تنظیمات fetch قرار داده شود. در غیر این صورت، کوکی `_medusa_admin_token` ارسال نشده و با خطای 401/403 مواجه می‌شوید.
8. **ریدارکت‌های درگاه پرداخت (Hardcoded URLs):**
در پیاده‌سازی APIهای بازگشتی (Callback) مانند زرین‌پال، هرگز از آدرس‌های هاردکد شده مثل `http://localhost:3000` برای `res.redirect` استفاده نکنید. همیشه از متغیر `process.env.FRONTEND_URL` استفاده کنید.
9. **گچا 10: پوشه `static` بدون volume mapping**:
پوشه `my-medusa-backend/static/` که محل ذخیره تصاویر آپلودی است، در `docker-compose.yml` **volume mapping** نداشته و با هر deploy جدید، تمام تصاویر پاک می‌شوند. برای حفظ تصاویر باید در `docker-compose.yml` زیر سرویس `backend` متصل شود.

**-- نکات جدید حاصل از دیباگینگ پیشرفته (آپدیت اخیر) --**

10. **تبدیل محصول ساده به متغیر (Inventory Reservation Lock):**
تبدیل یک محصول «ساده» (Simple) که دارای `Default Option` و `Default Variant` است، به یک محصول متغیر (Variable) در صورتی که سفارش ثبت‌شده، سبد خرید باز یا رزرو موجودی داشته باشد، مستقیماً از طریق کد غیرممکن است (خطای 400 `Cannot remove inventory items since they have reservations`). مدوسا اجازه حذف واریانت دیفالت قفل‌شده را نمی‌دهد. راهکار استاندارد: ساخت یک محصول کاملاً جدید به صورت متغیر از پایه و آرشیو کردن محصول قدیمی.
11. **ساختار Payload برای واریانت‌های جدید (حذف ارواح Default Option):**
هنگام ایجاد واریانت‌های جدید (مثلاً رنگ و سایز) از طریق API، به هیچ وجه نباید `Default Option` را در Payload شیء `options` قرار داد. مدوسا اگر آپشن دیفالت را همراه با آپشن‌های جدید دریافت کند خطای `invalid_data` پرتاب می‌کند (`Product has X option values but there were Y provided`).
12. **خطر انباشت قیمت‌های مخفی (Ghost Prices):**
هنگام آپدیت قیمت‌های تخفیف در `Price Lists` از طریق آدرس `batch` (`/admin/price-lists/{id}/prices/batch`)، اگر `id` قیمت قدیمی در آرایه `delete` ارسال نشود، مدوسا قیمت‌های قبلی را حذف نمی‌کند. این قیمت‌های قدیمی در دیتابیس می‌مانند و از آنجا که Store API همیشه پایین‌ترین قیمت محاسبه‌شده (`calculated_price`) را برمی‌گرداند، فرانت‌اند قیمت اشتباهی را به جای قیمت جدید ثبت شده نشان می‌دهد. رفع این مشکل نیازمند استخراج شناسه دقیق قیمت از Store API یا پاک‌سازی کامل Price List است.
13. **خطای 404 در Inventory Location Levels:**
وقتی واریانت جدید در مدوسا ایجاد می‌شود، یک `inventory_item_id` دریافت می‌کند اما به صورت خودکار به لوکیشن انبار (`location_level`) وصل نمی‌شود. در هنگام آپدیت موجودی، گرفتن خطای `404 Not Found` کاملاً طبیعی است و باید هندل شود؛ به این صورت که پس از دریافت 404، با یک متد `POST` سطح موجودی برای آن لوکیشن ایجاد (Create) گردد.
14. **پنهان‌سازی API Key در درخواست‌های Store API از داشبورد:**
هنگام فراخوانی `/store/products` از داخل پنل ادمین (برای دور زدن محدودیت‌های ادمین و دریافت لیست کامل تخفیف‌ها)، باید هدر `x-publishable-api-key` ارسال شود. بدون این هدر و یا ارسال ناقص آن، مدوسا دیتای مرتبط با ریجن (Region) و قیمت‌های محاسبه‌شده را مسدود می‌کند.
15. **مشکل تداخل کش (Cache) در Next.js برای قیمت‌ها:**
گاهی اوقات با وجود ثبت صحیح قیمت واریانت‌ها در بک‌اند مدوسا، فرانت‌اند قیمت‌ها را همچنان صفر یا قدیمی نمایش می‌دهد. این به دلیل ماهیت Aggressive Caching در App Router نسخه 14 است. راه حل این است که پوشه `.next` به طور کامل حذف شده و سرور مجدداً بیلد شود یا استراتژی‌های Revalidation به درستی تنظیم شوند.